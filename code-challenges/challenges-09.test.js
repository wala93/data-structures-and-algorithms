...
function transformToLis(obj){
  // Solution code here...
  const arr=[];
  let list= Object.keys(obj).map(function (key,value) {
    arr.push(`<li>${key}: ${Object.values(obj)[value]}</li>`);
  });
  return arr;
  //   console.log(arr);
...
const addValues = (arr) => {
  // Solution code here...
  let add= arr.reduce(function(accumulator, currentValue) {
...
const addPurchases = (arr) => {
  // Solution code here...
  let addP =arr.reduce(function(a,b){
    a= a+b.purchasePrice;
    return a;
  },0);
  // return addPurchase;
...
const countNumberOfElements = (arr) => {
  ...
  let counter=arr.reduce(function(indx){
    indx++;
    return indx;
  },0);
  return counter;
...
const reversedString = (str) => {
  ...
  let revStr=arr.reduce(function(a,b,idx){
    a.push(arr[arr.length-(idx+1)]);
    return a;
  },[]);
  let stringAfter= revStr.join('');
  return stringAfter;
